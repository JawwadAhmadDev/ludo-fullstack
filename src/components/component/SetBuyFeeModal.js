...
export default function SetBuyFeeModal() {
    ...
    const submitFee = async (e) => {
        ...
        var tx = await contract.methods.setBuyFees(pctToMaxPCT(liquidityFee), pctToMaxPCT(marketingFee), pctToMaxPCT(developmentFee), pctToMaxPCT(jackpotFee)).send({ from: walletStateValue.userWallet })
        console.log(tx)
                ...
                <Box sx={{ ...style }}>
                    {
                        isPendingTx ?
                            <div>
                                ...
                                <h4 className='alert  text-center' style={{ color: 'black' }}> Transection Mining </h4>
