...
export default function SetDevWallet() {
  ...
  const submitHandler = async (e) => {
    ...
    contract.methods.setBuybackWallet(addr).send({ from: walletStateValue.userWallet })
      .then(tx => {
        setOpen(false)
        toast(<a target="_blank" style={{ color: 'gray' }} href={`https://testnet.bscscan.com/tx/${tx.transactionHash}`}>Dev Wallet Added ! .🔗 View Tx On BSC Scan !!</a>, { autoClose: false })
        setAddr('')
      })
      .catch(err => {
        setAddr('')
